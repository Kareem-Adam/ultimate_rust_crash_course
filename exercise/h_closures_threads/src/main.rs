...
fn main() {
    ...
    let tx2: channel::Sender<&str> = tx.clone();
    ...
    //clone recieving channel 
    let rx2 = rx.clone();
    //create threads and print recieved message if received okay
    let handle_a: thread::JoinHandle<()> = thread::spawn(move || loop {
    ...
    let handle_b: thread::JoinHandle<()> = thread::spawn(move || loop {
    ...
    //iterate from 100 to 105 and print
    for i in 100..=105 {
        tx.send(i).unwrap();
        pause_ms(20);
    }
